@if(Auth::guard('tukang')->check() && (!Auth::guard('tukang')->user()->latitude || !Auth::guard('tukang')->user()->longitude))
<div class="location-modal-overlay" id="locationModal">
    <div class="location-modal">
        <div class="location-modal-header">
            ...
            <h2 class="location-modal-title">Complete Your Profile</h2>
            <p class="location-modal-subtitle">
                Set your working location and details so customers can find you.
            </p>
        </div>

        <div class="location-modal-body">
            <div class="location-content-grid">
                <!-- Left Column: Details Input Fields -->
                <div class="location-form-column">
                    <div class="location-form-section">
                        <div class="location-form-label">Professional Details</div>
                        ...
                        <div class="location-input-group">
                            ...
                            <small class="location-input-error" id="experienceError" style="display: none;"></small>
                        </div>

                        <div class="location-input-group">
                            <div class="location-input-wrapper" style="align-items: flex-start; padding-top: 0.75rem;">
                                <i class="bi bi-file-text" style="margin-top: 0.25rem;"></i>
                                <textarea 
                                    id="descriptionInput" 
                                    class="location-input" 
                            ...
                            <small class="location-input-error" id="descriptionError" style="display: none;"></small>
                        </div>

                        <div class="location-form-label mt-4">Address Information</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initMap() {
        if (!mapElement) return; // Guard clause
        ...
        // Trigger resize to fix any layout issues since modal might be initially hidden/flex
        setTimeout(() => {
            map.invalidateSize();
        }, 300);
    ...
    function clearAllErrors() {
        ...
        clearFieldError(experienceInput, experienceError);
    ...
    async function updateLocation(lat, lng) {
        ...
        // Enable save button (basic validation will also check fields)
    ...
    async function saveLocation() {
        ...
        if (!experience) {
            showFieldError(experienceInput, experienceError, 'Please enter your years of experience');
            hasError = true;
        }

        if (!description) {
            showFieldError(descriptionInput, descriptionError, 'Please enter a description');
            hasError = true;
        }
        ...
        try {
            const response = await fetch('{{ route("tukang.profile.complete") }}', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                ...
                coordinatesError.textContent = data.message || 'Failed to save profile';
                coordinatesError.style.display = 'block';
                saveLocationBtn.disabled = false;
                saveLocationBtn.innerHTML = '<i class="bi bi-save"></i> Save Profile';
            }
        } catch (error) {
            console.error('Save error:', error);
            coordinatesError.textContent = 'An error occurred while saving your profile';
            coordinatesError.style.display = 'block';
            saveLocationBtn.disabled = false;
            saveLocationBtn.innerHTML = '<i class="bi bi-save"></i> Save Profile';
        }
    ...
    [addressInput, cityInput, postalCodeInput, experienceInput, descriptionInput].forEach(input => {
        input.addEventListener('input', () => clearFieldError(input, input.nextElementSibling));
    });
